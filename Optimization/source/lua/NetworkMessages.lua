-- ======= Copyright (c3) 2003-2010, Unknown Worlds Entertainment, Inc. All rights reserved. =======
--
...
-- Cache functions
local Shared = Shared
local RegisterNetworkMessage = Shared.RegisterNetworkMessage
local table = table
local math = math
local Vector = Vector
local Log = Log
...
-- For damage numbers
local kFloatRange = string.format("float (%d to %d by 0.05)", -kHitEffectMaxPosition, kHitEffectMaxPosition)

-- Fast surface enum lookup
-- Pre-compute and cache frequently accessed values
local kHitEffectSurfaceEnumMap = {}
local kHitEffectSurfaceStringMap = {}

-- Initialize the maps once at load time
local function InitHitEffectMaps()
    for name, value in pairs(kHitEffectSurface) do
        kHitEffectSurfaceEnumMap[name] = value
        kHitEffectSurfaceStringMap[value] = name
    end
end

InitHitEffectMaps()

-- Fast lookups without string operations
local function GetSurfaceEnum(surfaceName)
    return kHitEffectSurfaceEnumMap[surfaceName] or kHitEffectSurface.metal
end

local function GetSurfaceString(surfaceEnum)
    return kHitEffectSurfaceStringMap[surfaceEnum] or "metal"
end
...
local function getMessageFromPool()
    return table.remove(messagePool) or {}
end

local function returnMessageToPool(message)
    table.wipe(message)
    table.insert(messagePool, message)
end

local kInitialAlienVisionStateMessage = {
    startsOn = "boolean"
}
RegisterNetworkMessage("InitAVState", kInitialAlienVisionStateMessage)
...
RegisterNetworkMessage("GlobalCommStats", kMarineCommStatsMessage)
RegisterNetworkMessage("MarineCommStats", kMarineCommStatsMessage) -- Only stats for Marine Commander... for now?
...
RegisterNetworkMessage("BuildingSummary", kBuildingSummaryMessage)

local kKillGraphMessage = {
    killerTeamNumber = "integer (1 to 2)",
    gameMinute = "float (0 to 1023 by 0.01)"
}
RegisterNetworkMessage("KillGraph", kKillGraphMessage)
...
RegisterNetworkMessage("RTGraph", kRTGraphMessage)
...
RegisterNetworkMessage("HiveSkillGraph", kHiveSkillGraphMessage)
...
local kTechLogMessage = {
    ...
    teamRes = table.concat({"float (0 to ", kMaxTeamResources, " by 0.01)"}),
    destroyed = "boolean",
    built = "boolean",
    recycled = "boolean"
}
RegisterNetworkMessage("TechLog", kTechLogMessage)
...
RegisterNetworkMessage("GameData", kGameDataMessage)

local kPlayerStatsMessage = {
    isMarine = "boolean",
    playerName = table.concat({"string (", kMaxNameLength * 4, ")"}),
    kills = table.concat({"integer (0 to ", kMaxKills, ")"}),
    assists = table.concat({"integer (0 to ", kMaxKills, ")"}),
    deaths = table.concat({"integer (0 to ", kMaxDeaths, ")"}),
    score = table.concat({"integer (0 to ", kMaxScore, ")"}),
...
RegisterNetworkMessage("DisabledOption", kDisabledOptionMessage)

local kAlienWeaponUseHUDSlotMessage = {
    slotMode = "integer (0 to 3)"
}
RegisterNetworkMessage("SetAlienWeaponUseHUDSlot", kAlienWeaponUseHUDSlotMessage)

local kAutopickupMessage = {
    autoPickup = "boolean",
    autoPickupBetter = "boolean"
}
RegisterNetworkMessage("SetAutopickup", kAutopickupMessage)

local kServerConfirmedHitEffectsMessage = {
    serverBlood = "boolean"
}
RegisterNetworkMessage("ServerConfirmedHitEffects", kServerConfirmedHitEffectsMessage)

local kCameraShakeMessage = {
    intensity = "float (0 to 1 by 0.01)"
}

RegisterNetworkMessage("CameraShake", kCameraShakeMessage)
...
RegisterNetworkMessage("CreateDecal", kCreateDecalMessage)

local function BuildMessage(template, values)
    local t = {}
    for key, default in pairs(template) do
        t[key] = values[key] or default
    end
    return t
end
...
function ParseCreateDecalMessage(message)
    return GetVectorFromIndex(message.normal), Vector(message.posx, message.posy, message.posz),
        GetDecalMaterialNameFromIndex(message.decalIndex), message.scale
end
...
RegisterNetworkMessage("SetPlayerCallingCard", kSetPlayerCallingCardMessage)
...
RegisterNetworkMessage("SetPlayerVariant", kSetPlayerVariantMessage)
...
RegisterNetworkMessage("VoiceMessage", kVoiceOverMessage)

local kMaxDamagePerHit = 511
local kHitEffectMessage = {
    -- TODO: figure out a reasonable precision for the position.
    -- Devnull Todo: Change to vector if possible
    -- pos = "vector", -- Use a single vector instead of separate x,y,z
    posx = kFloatRange,
    posy = kFloatRange,
    posz = kFloatRange,
    doerId = "entityid",
...
function BuildHitEffectMessage(position, doer, surface, target, showtracer, altMode, damage, direction)
    ...
    t.surface = GetSurfaceEnum(surface) --(surface and StringToEnum(kHitEffectSurface, surface)) or kHitEffectSurface.metal
...
function ParseHitEffectMessage(message)
    ...
    local surface = GetSurfaceString(message.surface) --EnumToString(kHitEffectSurface, message.surface)
...
RegisterNetworkMessage("HitEffect", kHitEffectMessage)
...
function BuildDamageMessage(targetEntityId, amount, hitpos, type)
    assert(hitpos, "Hit position is required")
    assert(amount and amount >= 0, "Damage amount must be a non-negative number")

    local t = {}
    t.posx = hitpos.x
    t.posy = hitpos.y
    t.posz = hitpos.z
    t.amount = Clamp(amount, 0, 2048)
...
RegisterNetworkMessage("MarkEnemy", kMarkEnemyMessage)
...
RegisterNetworkMessage("HitSound", kHitSoundMessage)
...
RegisterNetworkMessage("AbilityResult", kAbilityResultMessage)
...
RegisterNetworkMessage("JoinError", kJoinErrorMessage)

-- Used to tell the server that a client has played the tutorial
RegisterNetworkMessage("PlayedTutorial", {})

RegisterNetworkMessage("CommanderLoginError", {})
...
RegisterNetworkMessage("CommanderPing", kCommanderPingMessage)
...
RegisterNetworkMessage("TechNodeUpdate", kTechNodeUpdateMessage)

local kMaxPing = 999

local kPingMessage = {
    clientIndex = "entityid",
    ping = table.concat({"integer (0 to ", kMaxPing, ")"})
}
...
RegisterNetworkMessage("Ping", kPingMessage)

kWorldTextMessageType = enum({'Resources', 'Resource', 'Damage', 'DamageBoneshield', 'CommanderError'})
kWorldTextDamageTypes = set {kWorldTextMessageType.Damage, kWorldTextMessageType.DamageBoneshield}
...
RegisterNetworkMessage("WorldText", kWorldTextMessage)
...
RegisterNetworkMessage("CommanderError", kCommanderErrorMessage)
...
RegisterNetworkMessage("SelectAndGoto", kSelectAndGotoMessage)
RegisterNetworkMessage("ComSelect", kSelectAndGotoMessage)
...
RegisterNetworkMessage("TakeDamageIndicator", kTakeDamageIndicator)
...
local kCreateHotkeyGroupMessage = {
    groupNumber = table.concat({"integer (1 to ", ToString(kMaxHotkeyGroups), ")"})
}
...
RegisterNetworkMessage("TechNodeInstance", kTechNodeInstanceMessage)

function ParseTechNodeInstanceMessage(techNode, networkVars)
    techNode.instances = techNode.instances or IterableDict()
    ...
    if techNode.instances[networkVars.entity] then
        ...
        techNode.instances[networkVars.entity] = {
            progress = networkVars.progress,
            researchId = networkVars.researchId,
            removed = networkVars.removed
        }
    end
end
...
RegisterNetworkMessage("SetName", kSetNameMessage)
...
RegisterNetworkMessage("GameEnd", kGameEndMessage)

RegisterNetworkMessage("EntityChanged", kEntityChangedMessage)
RegisterNetworkMessage("ResetGame", {})

-- Selection
RegisterNetworkMessage("SelectUnit", kSelectUnitMessage)
RegisterNetworkMessage("SelectHotkeyGroup", kSelectHotkeyGroupMessage)

-- Commander actions
RegisterNetworkMessage("CommAction", kCommAction)
RegisterNetworkMessage("CommTargetedAction", kCommTargetedAction)
RegisterNetworkMessage("CommTargetedActionWorld", kCommTargetedAction)
RegisterNetworkMessage("CreateHotKeyGroup", kCreateHotkeyGroupMessage)

-- Command Structure Events
RegisterNetworkMessage("DangerMusicUpdate", kDangerMusicUpdateMessage)

-- Notifications
RegisterNetworkMessage("MinimapAlert", kMinimapAlertMessage)
RegisterNetworkMessage("VoteConcedeCast", kVoteConcedeCastMessage)
RegisterNetworkMessage("VoteEjectCast", kVoteEjectCastMessage)
RegisterNetworkMessage("TeamConceded", kTeamConcededMessage)

-- Player actions
RegisterNetworkMessage("MutePlayer", kMutePlayerMessage)

-- Gorge select structure message
RegisterNetworkMessage("GorgeBuildStructure", kGorgeBuildStructureMessage)

-- Chat
RegisterNetworkMessage("ChatClient", kChatClientMessage)
RegisterNetworkMessage("Chat", kChatMessage)
RegisterNetworkMessage("ChatUnlocalized", kChatMessage)

-- Debug messages
RegisterNetworkMessage("DebugLine", kDebugLineMessage)
RegisterNetworkMessage("DebugCapsule", kDebugCapsuleMessage)

RegisterNetworkMessage("DumpRoundStats", kDebugDumpRoundEndStatsMessage)

-- Specific Entity Debug Tools
RegisterNetworkMessage("DebugGrenades", kDebugGrenades)

RegisterNetworkMessage("TechNodeBase", kTechNodeBaseMessage)
RegisterNetworkMessage("ClearTechTree", {})
...
RegisterNetworkMessage("SetCommunicationStatus", kCommunicationStatusMessage)
...
function BuildBuyMessage(techIds)
    ...
    local buyMessage = {
        techId1 = kTechId.None,
        techId2 = kTechId.None,
        techId3 = kTechId.None,
        techId4 = kTechId.None,
        techId5 = kTechId.None,
        techId6 = kTechId.None,
        techId7 = kTechId.None,
        techId8 = kTechId.None
    }
...
RegisterNetworkMessage("Buy", kBuyMessage)

local kAutoConcedeWarning = {
    time = "time",
    team1Conceding = "boolean"
}
RegisterNetworkMessage("AutoConcedeWarning", kAutoConcedeWarning)
...
RegisterNetworkMessage("ScoreUpdate", kScoreUpdate)

RegisterNetworkMessage("SetAchievement", {
    name = "string (48)"
})

RegisterNetworkMessage("SpectatePlayer", {
    entityId = "entityid"
})
RegisterNetworkMessage("SwitchFromFirstPersonSpectate", {
    mode = "enum kSpectatorMode"
})
RegisterNetworkMessage("SwitchFirstPersonSpectatePlayer", {
    forward = "boolean"
})
RegisterNetworkMessage("SetClientIndex", {
    clientIndex = "entityid"
})
RegisterNetworkMessage("ServerHidden", {
    hidden = "boolean"
})
RegisterNetworkMessage("SetClientTeamNumber", {
    teamNumber = string.format("integer (-1 to %d)", kRandomTeamType)
})
RegisterNetworkMessage("WaitingForAutoTeamBalance", {
    waiting = "boolean"
})
RegisterNetworkMessage("SetTimeWaveSpawnEnds", {
    time = "time"
})
RegisterNetworkMessage("SetIsRespawning", {
    isRespawning = "boolean"
})
RegisterNetworkMessage("SetDesiredSpawnPoint", {
    desiredSpawnPoint = "position"
})

local kTeamNumDef = table.concat({"integer (", kTeamInvalid, " to ", kSpectatorIndex, ")"})
RegisterNetworkMessage("DeathMessage", {
    killerIsPlayer = "boolean",
    killerId = "integer",
    killerTeamNumber = kTeamNumDef,
    iconIndex = "enum kDeathMessageIcon",
    targetIsPlayer = "boolean",
    targetId = "integer",
    targetTeamNumber = kTeamNumDef
})

RegisterNetworkMessage("DumpTeamBrain", {})

if Shared.GetThunderdomeEnabled() then
    -- End of round message to notify clients to check if they've unlocked any items
    RegisterNetworkMessage("Thunderdome_EndRoundItemsCheck")
    -- Notify clients the match is finalized and lobby state should be updated
    RegisterNetworkMessage("Thunderdome_MatchFinalized")
end

RegisterNetworkMessage("RoundStatsProcessingCompleted", {})

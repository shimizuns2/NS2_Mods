...
math_random() -- Call it once to "warm up" if necessary

-- math.random() is more random the more you call it. Don't ask.
-- This is anyway genereated several times during load.. lowered from 10 to 20
--for i = 1, 20 do
 --   math.random()
--end
...
function OnPhysicsTrigger(enterObject, triggerObject, enter)
    ...
    if not enterObject or not triggerObject then
        return
    end
    ...
    if not enterEntity or not triggerEntity then
        return
    end

    if enterEntity and triggerEntity then
        if enter then
            if enterEntity.OnTriggerEntered then
                enterEntity:OnTriggerEntered(enterEntity, triggerEntity)
            end
            if triggerEntity.OnTriggerEntered then
                triggerEntity:OnTriggerEntered(enterEntity, triggerEntity)
            end
        else
            if enterEntity.OnTriggerExited then
                enterEntity:OnTriggerExited(enterEntity, triggerEntity)
            end
            if triggerEntity.OnTriggerExited then
...
function Shared.TraceViewBox(x, y, roll, startPoint, endPoint, mask, filter)
    ...
    local transformedVectors = {}
    for dx = -1, 1 do
        for dy = -1, 1 do
            table.insert(transformedVectors, coords:TransformVector(Vector(dx * x, dy * y, 0)))
        end
    end

    for _, v1 in ipairs(transformedVectors) do
        local p1 = startPoint + v1
        local p2 = endPoint + v1
        trace = Shared.TraceRay(p1, p2, CollisionRep.Default, mask, filter)
        if trace.fraction < 1 then
            if not shortestTrace or shortestTrace.fraction > trace.fraction then
                shortestTrace = trace
            end
        end
        if Shared.DbgTraceViewBox then
    ...
    if shortestTrace then
        return makeResult(shortestTrace.fraction, startPoint + (endPoint - startPoint) * shortestTrace.fraction,
            shortestTrace.normal, shortestTrace.entity)
    end

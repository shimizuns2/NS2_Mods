...
Script.Load("lua/DebugUtility.lua")
Script.Load("lua/Closure.lua")
...
local kLastBuildVersion
function GetLastBuildVersion() -- Gets last build version, from application launch, NOT VM start
    if not kLastBuildVersion then
        kLastBuildVersion = Client and Client.GetOptionInteger("lastLoadedBuild", 0) or 0 -- Servers have a json file for config.
    end

    return kLastBuildVersion
end
...
function EntityFilterAllButIsaList(classnameList)
    return function(test)
        return table.icontains(classnameList, test:GetClassName())
    end
end
...
function EntityFilterByTeam(teamNumber)
    return function(test)
        return HasMixin(test, "Team") and test:GetTeamNumber() == teamNumber
    end
end

function EntityFilterByPlayableTeams()
    return function(test)
        return HasMixin(test, "Team") and
                   (test:GetTeamNumber() == kMarineTeamType or test:GetTeamNumber() == kAlienTeamType)
    end
end
...
function AddMaterialEffect(entity, material, viewMaterial, entities)
    ...
    while #stack > 0 do
        local currentEntity = table.remove(stack)

        if HasMixin(currentEntity, "Model") then
            local model = currentEntity._renderModel
            if model ~= nil then
                ...
                table.insert(entities, currentEntity:GetId())
            end
        end

        for i = 1, currentEntity:GetNumChildren() do
            table.insert(stack, currentEntity:GetChildAtIndex(i - 1))
        end
    end
end
...
function RemoveMaterialEffect(entities, material, viewMaterial)
    for i = 1, #entities do
        ...
        if not entity or not HasMixin(entity, "Model") then
            goto continue
        end
...
function GetLocalSteamID64()
    return Shared.ConvertSteamId32To64(Client.GetSteamId())
end

function GetLocalAvatarTextureName()
    return Client.GetTextureNameForAvatar(GetLocalSteamID64())
end

function GetEnumCount(enumTable)
    return table.icount(enumTable)
end
...
function DebugTraceBox(extents, startPoint, endPoint, lifetime, r, g, b, a)
    ...
    DebugLine(startPoint, endPoint, lifetime, r, g, b, a)

    local points = {}
    -- create points for the boxes around the start and endpoint
    for i = 0, 7 do
        local v = Vector(extents.x * (bit.band(i, 1) == 1 and 1 or -1), extents.y * (bit.band(i, 2) == 2 and 1 or -1),
...
function _DebugTraceNeighbours(lineArgs, points, pi, ...)
    ...
    for _, pointIndex in ipairs({...}) do
        ...
        DebugLine(p1, p2, lineArgs[1], lineArgs[2], lineArgs[3], lineArgs[4], lineArgs[5])
...
function DebugCapsule(sweepStart, sweepEnd, capsuleRadius, capsuleHeight, lifetime, forceSharedAPI)
    ...
    elseif Server then
        -- TODO - get rid of this eventually
        Server.SendNetworkMessage("DebugCapsule", BuildDebugCapsuleMessage(sweepStart, sweepEnd, capsuleRadius,
            capsuleHeight, lifetime), true)
    end
...
function Logger(name, logTable, enabled)
    local result = function(format, ...)
        if logTable[name] then
            Log(format, ...)
        end
    end
...
function GetDisplayNameForTechId(techId, defaultText)
    ...
    PROFILE("GetDisplayNameForTechId")
...
function LerpGeneric(startValue, targetValue, percentage)
    ...
    if type(startValue) == "table" then
        ...
        if table.icount(startValue) ~= table.icount(targetValue) then
            Print("LerpGeneric(): startValue and targetValue tables not the same size (%s, %s)", ToString(startValue),
...
local cDataTypes = {"Vector", "Color", "Trace", "Angles", "Coords"}
...
-- local ctint64, ctuint64 = 1ll, 1llu
...
function ToString(t)
    ...
    elseif type(t) == "cdata" then
        ...
        if ffi.istype("int64_t", t) or ffi.istype("uint64_t", t) then
            return tostring(t)
        end
...
function CopyCData(cdata)
    if cdata:isa("Vector") then
        return Vector(cdata)
    elseif cdata:isa("Angles") then
        return Angles(cdata)
    elseif cdata:isa("Coords") then
        return Coords(cdata)
    elseif cdata:isa("Color") then
        return Color(cdata)
    elseif cdata:isa("Trace") and Trace then
        return Trace(cdata)
    else
        return cdata
    end
...
function GetAnglesDifference(startAngle, endAngle)
    ...
    if (math.abs(diff) > 100) then
        Print("Warning - GetAnglesDiff(%.2f, %.2f) called with large numbers, should be optimized.", startAngle,
            endAngle)
    end
...
function DrawCoords(coords, larger)
...
function NetworkRandomInt(minValue, maxValue, logMessage)
    ...
    if gNetworkRandomLogData and gRandomDebugEnabled then
        ...
        local baseLogMessage = StringFormat("NetworkRandomInt(%s, %s) => %s", ToString(minValue), ToString(maxValue),
            ToString(result))
...
function url_encode(str)
    if (str) then
        ...
        str = string.gsub(str, "([^%w ])", function(c)
            return StringFormat("%%%02X", string.byte(c))
        end)
        str = string.gsub(str, " ", "+")
    end
    return str
...
function GetUniqueNameForPlayer(name, nameList)
    ...
    local function nameInTable(name)
...
function GetAndCheckValue(valueString, min, max, valueName, defaultValue, silent)
    ...
    if (numValue == nil) then
        ...
        if (not silent) then
            Shared.Message(StringFormat("GetAndCheckValue(%s): Value is nil, returning default of %s.", valueName,
                numValue))
        end
    ...
    elseif (numValue < min or numValue > max) then
        ...
        if (not silent) then
            Shared.Message(StringFormat("%s - Value %.2f is outside expected range (%.2f, %.2f), clamping to %.2f: ",
                valueName, numValue, min, max, clampedValue))
        end
...
local sharedlib = Shared or {}

local precacheLookup = {
    [".cinematic"] = sharedlib.PrecacheCinematic,
    [".model"] = sharedlib.PrecacheModel,
    [".material"] = sharedlib.PrecacheMaterial,
    [".animation_graph"] = sharedlib.PrecacheAnimationGraph,
    [".dds"] = sharedlib.PrecacheTexture,
    [".fnt"] = sharedlib.PrecacheFont,
    [".font"] = sharedlib.PrecacheFont,
    [".surface_shader"] = sharedlib.PrecacheSurfaceShader
}
...
function PrecacheAsset(effectName)
    ...
    if type(effectName) ~= "string" then
        ...
        error(string.format("PrecacheAsset(%s): effect name isn't a string (%s instead).", tostring(effectName),
            type(effectName)))
        return nil

    end

    local extidx = string.find(effectName, ".", 1, true)
    local precacher

    if extidx then
        -- Assume all almost all paths only have 1 dot
        if string.find(effectName, ".", extidx + 1, true) then
            for i = #effectName, 1, -1 do
                if effectName[i] == dot then
                    extidx = i
                    break
                end
            end
        end
        precacher = precacheLookup[string.lower(string.sub(effectName, extidx))]
    end

    if not precacher and StringStartsWith(effectName, "sound") then
        precacher = Shared.PrecacheSound
    end

    if precacher then
        precacher(effectName)
    else
...
function PrecacheMultipleAssets(effectName, substTable)
    local assets = {}
    for _, substString in ipairs(substTable) do
        table.insert(assets, StringFormat(effectName, substString))
    end
...
if Server then
    ...
    function CreateEntity(mapName, origin, teamNumber, extraValues, suppressError)
        ...
        if entity then
            ...
            if not suppressError then
                Log("CreateEntity(%s, %s, %s) returned nil.", ToString(mapName), ToString(origin), ToString(teamNumber))
                return nil
            else
                return nil
            end
...
local function MonitorCallHistoryHook(type)
    ...
    if type == "call" then
        ...
        local function generateOffset(offsetString, currentDepth)
            if currentDepth <= callstackDepth then
                return generateOffset(" " .. offsetString, currentDepth + 1)
            end
            return offsetString
        end
        ...
        local otherInfo = functionType .. " - " .. debug.getinfo(2, "S").short_src .. ":" ..
                              debug.getinfo(2, "l").currentline
        if table.icontains(allowedFunctionTypes, functionType) and
            not table.icontains(filterFunctionNames, functionName) then
...
function asserttype(varname, requiredType, var)
    ...
    if not typeMatches then
        Print("%s has type %s, required type %s, value %s", ToString(varname), ToString(type(var)),
            ToString(requiredType), ToString(var))
        assert(false)
    end
...
function ReplaceAllCharsInStringWith(str, char, repWith)
    ...
    while idx <= #outString do
        ...
        outString = string.format("%s%s%s", string.sub(outString, idx, findIdx - 1), repWith,
            string.sub(outString, findIdx + 1, #outString))
    end
...
local kInsaneChars =
    {{"+", "_plus"}, {"<", "_lt"}, {">", "_gt"}, {"|", "_pipe"}, {"\\", "_bckslsh"} -- TODO far from a complete list...
    }

function SanitizePathStringForOptionName(str)
...
function StringReformat(formatString, argTable)
    ...
    local orderedArgs = {}
    for a = 1, #argOrder do
        if not argTable[argOrder[a]] then
            return formatString
        end
...
function IsOnScreen(screenPosition)
    return screenPosition.x < Client.GetScreenWidth() and screenPosition.x > 0 and screenPosition.y <
               Client.GetScreenHeight() and screenPosition.y > 0
end
...
function ApplyAtmosphericDensity()
    ...
    if Client and Client.lightList and gAtmosphericsEnabled then
        ...
        local atmoModifier = Client.GetOptionFloat("graphics/atmospheric-density", kDefaultLightsAtmoDensity)
...
local kVectorList = { -- vec.y >= 0 88
-- vec.z >= 0 48
-- vec.x >= 0 26
-- vec.y >= 0.5  13
Vector(0.000000, 1.000000, 0.000000), Vector(0.000000, 0.955423, 0.295242), Vector(0.238856, 0.864188, 0.442863),
Vector(0.000000, 0.850651, 0.525731), Vector(0.147621, 0.716567, 0.681718), Vector(0.500000, 0.809017, 0.309017),
Vector(0.262866, 0.951056, 0.162460), Vector(0.850651, 0.525731, 0.000000), Vector(0.716567, 0.681718, 0.147621),
Vector(0.525731, 0.850651, 0.000000), Vector(0.000000, 0.525731, 0.850651), Vector(0.309017, 0.500000, 0.809017),
Vector(0.425325, 0.688191, 0.587785), Vector(0.688191, 0.587785, 0.425325), -- vec.y < 0.5  12
Vector(0.000000, 0.000000, 1.000000), Vector(0.525731, 0.000000, 0.850651), Vector(0.295242, 0.000000, 0.955423),
Vector(0.442863, 0.238856, 0.864188), Vector(0.162460, 0.262866, 0.951056), Vector(0.864188, 0.442863, 0.238856),
Vector(0.809017, 0.309017, 0.500000), Vector(0.681718, 0.147621, 0.716567), Vector(0.587785, 0.425325, 0.688191),
Vector(0.955423, 0.295242, 0.000000), Vector(1.000000, 0.000000, 0.000000), Vector(0.951056, 0.162460, 0.262866),
Vector(0.850651, 0.000000, 0.525731), -- vec.x < 0 21
-- vec.y >= 0.5 9
Vector(-0.309017, 0.500000, 0.809017), Vector(-0.147621, 0.716567, 0.681718), Vector(-0.850651, 0.525731, 0.000000),
Vector(-0.716567, 0.681718, 0.147621), Vector(-0.688191, 0.587785, 0.425325), Vector(-0.500000, 0.809017, 0.309017),
Vector(-0.238856, 0.864188, 0.442863), Vector(-0.262866, 0.951056, 0.162460), Vector(-0.425325, 0.688191, 0.587785),
Vector(-0.525731, 0.850651, 0.000000), -- vec.y < 0.5 11
Vector(-0.864188, 0.442863, 0.238856), Vector(-0.525731, 0.000000, 0.850651), Vector(-0.442863, 0.238856, 0.864188),
Vector(-0.295242, 0.000000, 0.955423), Vector(-0.162460, 0.262866, 0.951056), Vector(-0.681718, 0.147621, 0.716567),
Vector(-0.809017, 0.309017, 0.500000), Vector(-0.587785, 0.425325, 0.688191), Vector(-0.955423, 0.295242, 0.000000),
Vector(-0.951056, 0.162460, 0.262866), Vector(-0.850651, 0.000000, 0.525731), Vector(-1.000000, 0.000000, 0.000000),

-- vec.z < 0  39
-- vec.x >= 0 21
-- vec.y >= 0.5 10
Vector(0.716567, 0.681718, -0.147621), Vector(0.000000, 0.850651, -0.525731), Vector(0.000000, 0.955423, -0.295242),
Vector(0.238856, 0.864188, -0.442863), Vector(0.262866, 0.951056, -0.162460), Vector(0.500000, 0.809017, -0.309017),
Vector(0.147621, 0.716567, -0.681718), Vector(0.309017, 0.500000, -0.809017), Vector(0.425325, 0.688191, -0.587785),
Vector(0.688191, 0.587785, -0.425325), Vector(0.000000, 0.525731, -0.850651), -- vec.y < 0.5  10
Vector(0.850651, 0.000000, -0.525731), Vector(0.442863, 0.238856, -0.864188), Vector(0.587785, 0.425325, -0.688191),
Vector(0.000000, 0.000000, -1.000000), Vector(0.162460, 0.262866, -0.951056), Vector(0.295242, 0.000000, -0.955423),
Vector(0.864188, 0.442863, -0.238856), Vector(0.809017, 0.309017, -0.500000), Vector(0.951056, 0.162460, -0.262866),
Vector(0.525731, 0.000000, -0.850651), Vector(0.681718, 0.147621, -0.716567), -- vec.x < 0 17
-- vec.y >= 0.5 7
Vector(-0.500000, 0.809017, -0.309017), Vector(-0.716567, 0.681718, -0.147621), Vector(-0.238856, 0.864188, -0.442863),
Vector(-0.262866, 0.951056, -0.162460), Vector(-0.147621, 0.716567, -0.681718), Vector(-0.309017, 0.500000, -0.809017),
Vector(-0.688191, 0.587785, -0.425325), Vector(-0.425325, 0.688191, -0.587785), -- vec.y < 0.5 9
Vector(-0.587785, 0.425325, -0.688191), Vector(-0.525731, 0.000000, -0.850651), Vector(-0.442863, 0.238856, -0.864188),
Vector(-0.295242, 0.000000, -0.955423), Vector(-0.162460, 0.262866, -0.951056), Vector(-0.864188, 0.442863, -0.238856),
Vector(-0.951056, 0.162460, -0.262866), Vector(-0.809017, 0.309017, -0.500000), Vector(-0.681718, 0.147621, -0.716567),
Vector(-0.850651, 0.000000, -0.525731), -- vec.y < 0 72
-- vec.z >= 0  39
-- vec.x >= 0 21
-- vec.y >= 0.5 13
Vector(0.000000, -1.000000, 0.000000), Vector(0.000000, -0.850651, 0.525731), Vector(0.000000, -0.955423, 0.295242),
Vector(0.238856, -0.864188, 0.442863), Vector(0.262866, -0.951056, 0.162460), Vector(0.500000, -0.809017, 0.309017),
Vector(0.716567, -0.681718, 0.147621), Vector(0.525731, -0.850651, 0.000000), Vector(0.309017, -0.500000, 0.809017),
Vector(0.147621, -0.716567, 0.681718), Vector(0.000000, -0.525731, 0.850651), Vector(0.425325, -0.688191, 0.587785),
Vector(0.688191, -0.587785, 0.425325), Vector(0.850651, -0.525731, 0.000000), -- vec.y < 0.5 7
Vector(0.442863, -0.238856, 0.864188), Vector(0.162460, -0.262866, 0.951056), Vector(0.587785, -0.425325, 0.688191),
Vector(0.955423, -0.295242, 0.000000), Vector(0.864188, -0.442863, 0.238856), Vector(0.951056, -0.162460, 0.262866),
Vector(0.809017, -0.309017, 0.500000), Vector(0.681718, -0.147621, 0.716567), -- vec.x < 0  17
-- vec.y >= 0.5 9
Vector(-0.850651, -0.525731, 0.000000), Vector(-0.716567, -0.681718, 0.147621), Vector(-0.525731, -0.850651, 0.000000),
Vector(-0.500000, -0.809017, 0.309017), Vector(-0.238856, -0.864188, 0.442863), Vector(-0.262866, -0.951056, 0.162460),
Vector(-0.688191, -0.587785, 0.425325), Vector(-0.309017, -0.500000, 0.809017), Vector(-0.147621, -0.716567, 0.681718),
Vector(-0.425325, -0.688191, 0.587785), -- vec.y < 0.5 7
Vector(-0.162460, -0.262866, 0.951056), Vector(-0.951056, -0.162460, 0.262866), Vector(-0.955423, -0.295242, 0.000000),
Vector(-0.681718, -0.147621, 0.716567), Vector(-0.442863, -0.238856, 0.864188), Vector(-0.587785, -0.425325, 0.688191),
Vector(-0.864188, -0.442863, 0.238856), Vector(-0.809017, -0.309017, 0.500000), -- vec.z < 0  32
-- vec.x >= 0 17
-- vec.y >= 0.5 10
Vector(0.000000, -0.850651, -0.525731), Vector(0.262866, -0.951056, -0.162460), Vector(0.147621, -0.716567, -0.681718),
Vector(0.000000, -0.525731, -0.850651), Vector(0.309017, -0.500000, -0.809017), Vector(0.238856, -0.864188, -0.442863),
Vector(0.500000, -0.809017, -0.309017), Vector(0.425325, -0.688191, -0.587785), Vector(0.716567, -0.681718, -0.147621),
Vector(0.688191, -0.587785, -0.425325), Vector(0.000000, -0.955423, -0.295242), -- vec.y < 0.5 6
Vector(0.442863, -0.238856, -0.864188), Vector(0.162460, -0.262866, -0.951056), Vector(0.587785, -0.425325, -0.688191),
Vector(0.681718, -0.147621, -0.716567), Vector(0.864188, -0.442863, -0.238856), Vector(0.809017, -0.309017, -0.500000),
Vector(0.951056, -0.162460, -0.262866), -- vec.x < 0 14
-- vec.y >= 0.5  7
Vector(-0.238856, -0.864188, -0.442863), Vector(-0.500000, -0.809017, -0.309017),
Vector(-0.262866, -0.951056, -0.162460), Vector(-0.716567, -0.681718, -0.147621),
Vector(-0.425325, -0.688191, -0.587785), Vector(-0.688191, -0.587785, -0.425325),
Vector(-0.147621, -0.716567, -0.681718), Vector(-0.309017, -0.500000, -0.809017), -- vec.y < 0.5 6
Vector(-0.442863, -0.238856, -0.864188), Vector(-0.162460, -0.262866, -0.951056),
Vector(-0.864188, -0.442863, -0.238856), Vector(-0.951056, -0.162460, -0.262866),
Vector(-0.809017, -0.309017, -0.500000), Vector(-0.681718, -0.147621, -0.716567),
...
function IsValid(obj)
    return obj ~= nil and (debug.isvalid(obj))
end
...
function HexToColor(hx, alpha)
    ...
    -- Remove 0x or # prefix... yea, not bulletproof but should work well enough.
    hx = string.gsub(hx, "0x", "")
    hx = string.gsub(hx, "#", "")

    hx = "0x" .. hx -- add 0x prefix (ensuring it's only there once.)
...
local kSpaceAdditions = {'Num', 'Pad', 'Left', 'Right', 'Page', 'App', 'Mouse', 'Button', 'Wheel', 'Print', 'Joystick',
                         'Rotation', 'Pov', 'Slider'}
function GetFriendlyBindingName(name)

    local newName = name
    for i = 1, #kSpaceAdditions do
        newName, _ = string.gsub(newName, kSpaceAdditions[i], kSpaceAdditions[i] .. ' ')
    end
...
local function GetXYDistanceSquared(coords, origin)

    local diff = origin - coords.origin
    local x = diff:DotProduct(coords.xAxis)
    local y = diff:DotProduct(coords.yAxis)
    return x * x + y * y

end

local _xyCompareCoords
local function CompareXYDistance(a, b)
    local dist1 = GetXYDistanceSquared(_xyCompareCoords, a:GetOrigin())
    local dist2 = GetXYDistanceSquared(_xyCompareCoords, b:GetOrigin())
    return dist1 < dist2
end
...
function SortEntitiesBasedOn2DXYDistance(viewCoords, list)
    ...
    for i, entity in ipairs(list) do
        distances[i] = { entity = entity, distance = GetXYDistanceSquared(viewCoords, entity:GetOrigin()) }
    end
...
function DebugFunctionToString(func)
    ...
    if infoTable.namewhat == "" then
        ...
        result = string.format("%s function named %s, defined at line %s of %s", infoTable.namewhat, infoTable.name,
            infoTable.linedefined, infoTable.short_src)
    end
...
---@param point Vector coordinates  of point to be tested
---@param coneOrigin Vector coordinates of apex point of cone
---@param coneDir Vector coordinates of center of cone's basement circle
---@param angle Angle cone's aperture in radians
function IsPointInCone(point, coneOrigin, coneDir, angle)

    -- Vector pointing to camera point from cone point
    local apexToXVect = coneOrigin - point;

    -- Vector pointing from apex to circle-center point.
    local axisVect = -coneDir;
    ...
    local isInInfiniteCone = Math.DotProduct(apexToXVect, axisVect) / apexToXVect:GetLength() / axisVect:GetLength() >

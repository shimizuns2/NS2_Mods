...
function NS2Gamerules_GetUpgradedDamageScalar(attacker, weaponTechId)
    ...
    if not upgradedDamageScalars then
        ...
        upgradedDamageScalars = {
            [kTechId.Shotgun] = {kShotgunWeapons1DamageScalar, kShotgunWeapons2DamageScalar,
                                 kShotgunWeapons3DamageScalar},
            [kTechId.GrenadeLauncher] = {kGrenadeLauncherWeapons1DamageScalar, kGrenadeLauncherWeapons2DamageScalar,
                                         kGrenadeLauncherWeapons3DamageScalar},
            [kTechId.Flamethrower] = {kFlamethrowerWeapons1DamageScalar, kFlamethrowerWeapons2DamageScalar,
                                      kFlamethrowerWeapons3DamageScalar},
...
function NS2Gamerules_GetUpgradedDamage(attacker, doer, damage)
    ...
    if attacker ~= nil then
        ...
        if doer.GetIsAffectedByWeaponUpgrades and doer:GetIsAffectedByWeaponUpgrades() then
            damageScalar = NS2Gamerules_GetUpgradedDamageScalar(attacker, ConditionalValue(HasMixin(doer, "Tech"),
                doer:GetTechId(), kTechId.None))
        end
...
function NS2Gamerules_GetUpgradedAlienDamage(target, attacker, doer, damage, armorFractionUsed, _, damageType)
    ...
    local isAffectedByCrush = doer.GetIsAffectedByCrush and attacker:GetHasUpgrade(kTechId.Crush) and
                                  doer:GetIsAffectedByCrush()
    local isAffectedByVampirism = doer.GetVampiricLeechScalar and attacker:GetHasUpgrade(kTechId.Vampirism)
    local isAffectedByFocus = doer.GetIsAffectedByFocus and attacker:GetHasUpgrade(kTechId.Focus) and
                                  doer:GetIsAffectedByFocus()

    if isAffectedByCrush then -- Crush
        local crushLevel = attacker:GetSpurLevel()
        if crushLevel > 0 then
            if target:isa("Exo") or target:isa("Exosuit") or target.GetReceivesStructuralDamage and
                target:GetReceivesStructuralDamage(damageType) then
...
kDamageType = enum({'Normal', 'Light', 'Heavy', 'Puncture', 'Structural', 'StructuralHeavy', 'Splash', 'Gas',
                    'NerveGas', 'StructuresOnly', 'Falling', 'Door', 'Flame', 'Corrode', 'ArmorOnly', 'Biological',
                    'StructuresOnlyLight', 'Spreading', 'GrenadeLauncher', 'MachineGun', 'ClusterFlame',
                    'ClusterFlameFragment'})

kDamageTriggerTypes = enum({'Normal', 'Gas', 'Fire'})

-- Describe damage types for tooltips
kDamageTypeDesc = {"", "Light: reduced vs. armor", "Heavy: extra vs. armor", "Puncture: extra vs. players",
                   "Structural: Double vs. structures", "StructuralHeavy: Double vs. structures and double vs. armor",
                   ...
                   "Falling: Ignores armor for humans, no damage for aliens", "Door: Can also affect Doors",
                   ...
                   "Armor damage: Will never reduce health", "Biological: Will only damage biological targets.",
...
local function ApplyHighArmorUseFractionForExos(target, _, _, damage, armorFractionUsed, healthPerArmor, _, _, _,
    overshieldDamage)
...
local function ApplyAttackerModifiers(_, attacker, doer, damage, armorFractionUsed, healthPerArmor, damageType,
    hitPoint, _, overshieldDamage)
    ...
    if attacker and attacker.ComputeDamageAttackerOverride then
        damage, overshieldDamage = attacker:ComputeDamageAttackerOverride(attacker, damage, damageType, doer, hitPoint,
            overshieldDamage)
    end

    if doer and doer.ComputeDamageAttackerOverride then
        damage, overshieldDamage = doer:ComputeDamageAttackerOverride(attacker, damage, damageType, doer, hitPoint,
            overshieldDamage)
    end

    if attacker and attacker.ComputeDamageAttackerOverrideMixin then
        damage, overshieldDamage = attacker:ComputeDamageAttackerOverrideMixin(attacker, damage, damageType, doer,
            hitPoint, overshieldDamage)
    end

    if doer and doer.ComputeDamageAttackerOverrideMixin then
        damage, overshieldDamage = doer:ComputeDamageAttackerOverrideMixin(attacker, damage, damageType, doer, hitPoint,
            overshieldDamage)
    end
...
local function ApplyTargetModifiers(target, attacker, doer, damage, armorFractionUsed, healthPerArmor, damageType,
    hitPoint, weapon, overshieldDamage)

    -- The host can provide an override for this function.
    if target.ComputeDamageOverride then
        damage, overshieldDamage =
            target:ComputeDamageOverride(attacker, damage, damageType, hitPoint, overshieldDamage)
    end

    -- Used by mixins.
    if target.ComputeDamageOverrideMixin then
        damage, overshieldDamage = target:ComputeDamageOverrideMixin(attacker, damage, damageType, hitPoint,
            overshieldDamage)
    end

    if target.ShieldComputeDamageOverrideMixin then
        damage, overshieldDamage = target:ShieldComputeDamageOverrideMixin(attacker, damage, damageType, hitPoint,
            overshieldDamage)
    end
...
local function ApplyFriendlyFireModifier(target, attacker, _, damage, armorFractionUsed, healthPerArmor, _, _, _,
    overshieldDamage)

    if target and attacker and target ~= attacker and HasMixin(target, "Team") and HasMixin(attacker, "Team") and
        target:GetTeamNumber() == attacker:GetTeamNumber() then
...
local function MultiplyForPlayers(target, _, _, damage, armorFractionUsed, healthPerArmor)
    return
        ConditionalValue(target:isa("Player") or target:isa("Exosuit"), damage * kPuncturePlayerDamageScalar, damage),
        armorFractionUsed, healthPerArmor
end
...
local function DamageAlienOnly(target, _, _, damage, armorFractionUsed, healthPerArmor)
    return ConditionalValue(HasMixin(target, "Team") and target:GetTeamType() == kAlienTeamType, damage, 0),
        armorFractionUsed, healthPerArmor
end
...
local function MultiplyForMachineGun(target, _, _, damage, armorFractionUsed, healthPerArmor)
    return ConditionalValue(target:isa("Player") or target:isa("Exosuit"), damage * kMachineGunPlayerDamageScalar,
        damage), armorFractionUsed, healthPerArmor
end
...
local function BuildDamageTypeRules()
    ...
    -- global rules
    kDamageTypeGlobalRules = {ApplyDefaultArmorUseFraction, ApplyHighArmorUseFractionForExos,
                              ApplyDefaultHealthPerArmor, ApplyAttackerModifiers, ApplyTargetModifiers,
    ...
    -- light damage rules
    kDamageTypeRules[kDamageType.Light] = {DoubleHealthPerArmor}
    -- ------------------------------

    -- heavy damage rules
    kDamageTypeRules[kDamageType.Heavy] = {HalfHealthPerArmor}
    -- ------------------------------

    -- Puncture damage rules
    kDamageTypeRules[kDamageType.Puncture] = {MultiplyForPlayers}
    -- ------------------------------

    -- Spreading damage rules
    kDamageTypeRules[kDamageType.Spreading] = {ReducedDamageAgainstSmall}
    -- ------------------------------

    -- structural rules
    kDamageTypeRules[kDamageType.Structural] = {MultiplyForStructures}
    -- ------------------------------

    -- Grenade Launcher rules
    kDamageTypeRules[kDamageType.GrenadeLauncher] = {QuintupleForStructure}
    -- ------------------------------

    -- Machine Gun rules
    kDamageTypeRules[kDamageType.MachineGun] = {MultiplyForMachineGun}
    -- ------------------------------

    -- structural heavy rules
    kDamageTypeRules[kDamageType.StructuralHeavy] = {HalfHealthPerArmor, MultiplyForStructures}
    -- ------------------------------

    -- gas damage rules
    kDamageTypeRules[kDamageType.Gas] = {IgnoreArmor, DamageBreathingOnly}
    -- ------------------------------

    -- structures only rules
    kDamageTypeRules[kDamageType.StructuresOnly] = {DamageStructuresOnly}
    -- ------------------------------

    -- Splash rules
    kDamageTypeRules[kDamageType.Splash] = {DamageStructuresOnly}
    -- ------------------------------

    -- fall damage rules
    kDamageTypeRules[kDamageType.Falling] = {IgnoreArmor}
    -- ------------------------------

    -- Door damage rules
    kDamageTypeRules[kDamageType.Door] = {MultiplyForStructures, HalfHealthPerArmor}
    -- ------------------------------

    -- Flame damage rules
    kDamageTypeRules[kDamageType.Flame] = {MultiplyFlameAble, MultiplyForStructures}
    -- ------------------------------

    -- ClusterFlame damage rules
    kDamageTypeRules[kDamageType.ClusterFlame] = {ClusterFlameModifier}
    -- ------------------------------

    -- ClusterFlameFragment damage rules
    kDamageTypeRules[kDamageType.ClusterFlameFragment] = {ClusterFlameFragmentModifier}
    -- ------------------------------

    -- Corrode damage rules
    kDamageTypeRules[kDamageType.Corrode] = {ReduceGreatlyForPlayers, IgnoreHealthForPlayersUnlessExo}
    -- ------------------------------

    -- nerve gas rules
    kDamageTypeRules[kDamageType.NerveGas] = {DamageAlienOnly, IgnoreHealth}
    -- ------------------------------

    -- StructuresOnlyLight damage rules
    kDamageTypeRules[kDamageType.StructuresOnlyLight] = {DoubleHealthPerArmorForStructures}
    -- ------------------------------

    -- ArmorOnly damage rules
    kDamageTypeRules[kDamageType.ArmorOnly] = {IgnoreHealth}
    -- ------------------------------

    -- Biological damage rules
    kDamageTypeRules[kDamageType.Biological] = {DamageBiologicalOnly}
    -- ------------------------------
...
function GetDamageByType(target, attacker, doer, damage, damageType, hitPoint, weapon)
    ...
    if not CanEntityDoDamageTo(attacker, target, Shared.GetCheatsEnabled(), Shared.GetDevMode(), GetFriendlyFire(),
        damageType) then
        return 0, 0, 0, 0
    end
    ...
    for _, rule in ipairs(kDamageTypeGlobalRules) do
        damage, armorFractionUsed, healthPerArmor, overshieldDamage =
            rule(target, attacker, doer, damage, armorFractionUsed, healthPerArmor, damageType, hitPoint, weapon,
                overshieldDamage)
    end
    ...
    if attacker:GetTeamType() == kAlienTeamType and attacker:isa("Player") then
        damage, armorFractionUsed = NS2Gamerules_GetUpgradedAlienDamage(target, attacker, doer, damage,
            armorFractionUsed, healthPerArmor, damageType, hitPoint, weapon)
    end

    if not kDamageTypeRules[damageType] then
        Log("Warning: Invalid damageType '%s'", tostring(damageType))
        return 0, 0, 0, 0
    end

    -- apply damage type specific rules
    for _, rule in ipairs(kDamageTypeGlobalRules) do
        damage, armorFractionUsed, healthPerArmor = rule(target, attacker, doer, damage, armorFractionUsed,
            healthPerArmor, damageType, hitPoint)
    end

    if target.ModifyDamageTakenPostRules then
        local damageTable = {
            damage = damage,
            armorFractionUsed = armorFractionUsed,
            healthPerArmor = healthPerArmor
        }
        local modifiedDamage, modifiedArmorFraction, modifiedHealthPerArmor =
            target:ModifyDamageTakenPostRules(damageTable, attacker, doer, damageType, hitPoint)
        damage = modifiedDamage or damage
        armorFractionUsed = modifiedArmorFraction or armorFractionUsed
        healthPerArmor = modifiedHealthPerArmor or healthPerArmor
    end

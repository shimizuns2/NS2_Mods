...
for _, asset in ipairs(assetsToPrecache) do
    PrecacheAsset(asset)
end

-- Script variables
local kMaxBiomassLevel = 12
...
function Alien:UpdateArmorAmount(carapaceLevel, biomassLevel)
    ...
    if carapaceLevel > 0 then
        newMaxArmor = newMaxArmor + (carapaceBaseArmorBonus + carapaceBiomassArmorBonus) * (carapaceLevel / 3)
    end
...
function Alien:GetUpgradeLevel(upgradeIndexName)
    ...
    if teamInfo then
        ...
        Log("Warning: TeamInfo is nil for teamNumber %d", self:GetTeamNumber())
    end
...
function Alien:GetControllerPhysicsGroup()
    if self and self.isHallucination then
...
function Alien:HandleButtons(input)
    if not input or not input.commands then
        return
    end
...
function Alien:GetArmorFullyUpgradedAmount()
    ...
    if teamEnt then
        return self:GetBaseArmor() + self:GetBaseCarapaceArmorBuff() + (self:GetCarapaceBonusPerBiomass() * kMaxBiomassLevel)
    end
